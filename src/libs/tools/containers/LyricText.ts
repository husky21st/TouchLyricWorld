...
import { IChar, IPhrase, ValenceArousalValue } from 'textalive-app-api';
...
export class LyricText extends Container {
  ...
  constructor(
    ...
    firstPhrase: IPhrase,
    allLyricText: string,
    vocalAmplitudes: Array<number>,
    valenceArousals: Array<ValenceArousalValue>,
    maxVocalAmplitude: number,
    medianValenceArousal: ValenceArousalValue,
  ) {
    super();
  ...
  private createTexts(
    ...
    const max16up: Array<number> = max28up.concat([
      2, 3, 12, 13, 22, 23, 32, 33, 41, 42, 51, 52, 61, 71, 81, 91,
    ]); // 11 ~ 16
    ...
    let vocalIndex: number = 0;

    p = firstPhrase;
    for (let i = 0; p !== null; i++) {
      let c: IChar = p.firstChar;
      for (let j = 0; j < p.charCount; j++, vocalIndex++) {
        const volumeLevel: number = 2.7 + (0.7 * vocalAmplitudes[vocalIndex]) / maxVocalAmplitude;
        const valenceA: number = valenceArousals[vocalIndex].a - medianValenceArousal.a;
        const valenceV: number = valenceArousals[vocalIndex].v - medianValenceArousal.v;
        const bgColor: number = utils.rgb2hex([
          valenceA * 0.9 + 0.1,
          valenceA * valenceV * 80 + 0.82,
          valenceV * 0.9 + 0.8,
        ]);
        ...
        const charText: BitmapText = new BitmapText(c.text, {
          fontName: 'BasicYuseiMagic',
          tint: 0x000000,
          fontSize: 64,
        });
        ...
        const charTextBG: BitmapText = new BitmapText(c.text, {
          fontName: 'BasicYuseiMagic',
          tint: bgColor,
          fontSize: 64,
        });
        ...
        textBG1.on(
          ...
          (event: InteractionEvent) => {
            this.addScore(event, newCharTextBox);
          },
          this,
        );
        textBG1.on(
      ...
      const phraseText: BitmapText = new BitmapText(p.text, {
        fontName: 'BasicYuseiMagic',
        tint: 0xfa0fd3,
        fontSize: 68,
      });
      ...
      const phraseTextBG: BitmapText = new BitmapText(p.text, {
        fontName: 'BasicYuseiMagic',
        tint: 0xffffff,
        fontSize: 68,
      });
